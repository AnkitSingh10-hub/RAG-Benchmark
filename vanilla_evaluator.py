...
from vanilla_evaluation.eval import evaluate_all_retrieval, evaluate_all_answers
...
def main():
    ...
    with gr.Blocks(title="RAG Evaluation Dashboard", theme=theme) as app:
        gr.Markdown(
            "# 📊 RAG Evaluation Dashboard (LangChain / Vanilla Implementation)"
        )
        gr.Markdown(
            "Evaluate retrieval and answer quality for the Insurellm RAG system "
            "(`implementation/` — LangChain-based)"
        )
        ...
        with gr.Accordion("⚙️ Run Configuration", open=True):
            ...
            with gr.Row():
                ...
                vector_db_input = gr.Textbox(
                    label="Vector DB", value="implementation/vector_database"
                )
            with gr.Row():
                generation_model_input = gr.Textbox(
                    label="Generation Model", value="gpt-5.6-luna"
                )
                judge_model_input = gr.Textbox(
                    label="Judge Model", value="DeepSeek-V4-Flash"
                )
